import type {
  InsertNewsDataIngestionLogInput,
  NewsDataRepository,
  UpdateNewsDataQueryGroupScheduleInput,
  UpsertNewsDataArticleMetadataInput
} from "@/application/ports/repositories/NewsDataRepository";
import { compareDueQueryGroups } from "@/application/services/news/gdeltQueryOrdering";
import type { NewsCanonicalTheme, NewsDataIngestionLog, NewsDataQueryGroup } from "@/domain/news/types";
...
function missing(error: { code?: string; message?: string } | null) {
  ...
  return Boolean(error && (error.code === "42P01" || message.includes("newsdata_")));
}
...
function mapQueryGroup(row: any): NewsDataQueryGroup {
  return {
    id: row.id,
...
function mapLog(row: any): NewsDataIngestionLog {
  return {
    id: row.id,
...
export class SupabaseNewsDataRepository implements NewsDataRepository {
  ...
  async updateQueryGroupSchedule(input: UpdateNewsDataQueryGroupScheduleInput) {
  ...
  async upsertArticleMetadata(input: UpsertNewsDataArticleMetadataInput[]) {
    ...
    const { error } = await this.db.from("newsdata_article_metadata").upsert(
      input.map((item) => ({
  ...
  async insertIngestionLog(input: InsertNewsDataIngestionLogInput) {
    const { error } = await this.db.from("newsdata_ingestion_logs").insert({
...
export const newsDataRepositoryInternals = { toStringArray };
