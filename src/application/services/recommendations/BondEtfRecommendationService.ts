...
export class BondEtfRecommendationService {
  ...
  evaluate(input: RecommendationInput) {
    ...
    const components: ScoreComponent[] = [
      { key: "duration_fit", label: "Duration Fit", score: scoreDurationFit(input), weight: 0.22, reason: "Duration profile fits the portfolio role" },
      { key: "rate_regime", label: "Rate Regime", score: scoreRateRegime(input), weight: 0.22, reason: "Rate regime supports this duration" },
      { key: "inflation_regime", label: "Inflation Regime", score: scoreInflationRegime(input), weight: 0.16, reason: "Inflation regime supports bond role" },
      { key: "yield_curve", label: "Yield Curve", score: scoreMacroFit(input.instrument, input.macroRegime, input.bondProfile), weight: 0.13, reason: "Yield-curve context supports the holding" },
      { key: "credit_risk", label: "Credit Risk", score: creditScore, weight: 0.11, reason: "Credit quality is controlled" },
      { key: "portfolio_stability", label: "Portfolio Stability", score: stabilityScore, weight: 0.11, reason: "Adds portfolio stability" },
