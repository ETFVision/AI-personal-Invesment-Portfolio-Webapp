...
- Risk Analytics diversification scoring measures breadth and correlation only: meaningful direct holdings, asset-class spread, sector spread, currency spread, and average correlation. Concentration is measured in the Concentration section so the two dimensions are not double-counted.
...
- `insufficient_real_estate_exposure` REIT candidates reference real-estate look-through exposure directly and must not expose internal issue-category slugs in card text.

The candidate logic should not change internal scoring labels. It uses stored insight outputs and active universe data as inputs into portfolio-level balance findings. Category-remedy findings surface diversified funds where appropriate; `insufficient_defensive_exposure` excludes individual single-stock instruments so Defensive Sectors examples are sector/diversified ETF instruments rather than individual company names. The Defensive Sectors finding presents candidates in per-sleeve subsections ordered by the most-underweight defensive sleeve first, with up to two examples per sleeve. The International Equity finding uses the same subsection presentation pattern with up to two candidates in Broad ex-US (all regions), Developed markets, and Emerging markets groups; the Broad ex-US group is framed as an all-in-one ex-US option rather than an additive peer to the regional building blocks. The Real Estate finding is a single-sleeve flat list that leads with broad US REIT representatives. It excludes narrow healthcare sub-theme ETFs such as XBI, IBB, and ARKG from this defensive-sector finding, and it prefers broad defensive sector ETFs such as XLV/VHT, XLU/VPU, and XLP/VDC within their respective sleeves. Defensive candidate tooltips should use the candidate's sleeve category, such as Utilities, Consumer Staples, or Healthcare, rather than a generic defensive category. User-facing cards should include the disclaimer chip: `Shown because category is lightly represented - not a buy recommendation`.
...
| `insufficient_international_exposure` | `usExposure > 0.7` OR `internationalExposure < 0.3` | high if usExposure > 0.85; medium if usExposure > 0.7; low otherwise |
...
| `insufficient_real_estate_exposure` | `realEstateWeight < 0.03` AND at least one eligible REIT candidate is available | low |
| `excessive_crypto_risk` | `cryptoAllocation > 0.05` AND `recessionHedgeAllocation < cryptoAllocation` | high if cryptoAllocation > 0.1; medium otherwise |
...
| `insufficient_real_estate_exposure` | real_estate |
...
- `insufficient_real_estate_exposure`: instruments classified to the `real_estate` role are eligible; the broad US REIT representatives `VNQ`, `SCHH`, `IYR`, `USRT`, `FREL`, `XLRE`, and `RWR` receive the highest category-representative score. Mortgage, international, and global REIT variants such as `REM`, `VNQI`, and `REET` rank below broad US representatives.
...
Up to 5 candidates are returned per non-defensive gap finding, sorted by `rankScore` descending. On the Portfolio Review page, those already-selected candidates are displayed by category fit (`categoryRepresentativeScore`, then `issueFitScore`, then `recommendationScore`) so broad/core instruments for the underweighted category appear first. For `insufficient_international_exposure`, broad ex-US representatives from the curated `GLOBAL_EQUITY`, `DEVELOPED_MARKETS`, and `EMERGING_MARKETS` ETF categories are tiered by breadth: core ex-US funds score 100, hedged/style/regional/subset variants score 60, and global-including-US funds score 30 because they reintroduce US look-through. Country and international-dividend funds remain at 0. International candidate selection returns up to two candidates per ex-US sub-role, grouped as Broad ex-US (all regions), Developed markets, and Emerging markets in that order; the Broad ex-US group is an all-in-one ex-US option, while the developed and emerging groups are regional building blocks. For `insufficient_defensive_exposure`, candidates are selected by equity sector sleeve only: up to two Utilities, Consumer Staples, or Healthcare candidates per sleeve, with sleeve order following the same most-underweight role priority used for `issueFitScore`. Narrow healthcare sub-theme ETFs such as XBI, IBB, and ARKG are excluded from the Defensive Sectors finding, while broad sleeve representatives such as XLV/VHT, XLU/VPU, and XLP/VDC are preferred within the per-sleeve ordering. For `insufficient_real_estate_exposure`, up to four candidates are returned in a flat list, using the same category-remedy ranking so broad US REIT funds lead mortgage, international, or global REIT variants. Instrument quality remains visible as a per-card badge, but display order is category-intrinsic rather than a personalised ranking.
