...
export async function POST(request: NextRequest) {
  ...
  return runCronJob(request, { jobName: "instrument-market-metrics-refresh", lockTtlSeconds, onSuccess: () => revalidateTag("market-data") }, () =>
    createContainer().instrumentMarketService.refreshInstrumentMarketMetricsInBatches({
      batchSize,
      maxBatches
    })
  );
