...
export default async function FundamentalsPage({ searchParams }: { searchParams: Promise<{ message?: string; error?: string }> }) {
  ...
  const covered = rows.filter((row) => scoreBusinessQuality(row.latestScore) != null).length;
              ...
              <table className="min-w-full text-left text-sm">
                <thead className="border-b text-xs uppercase text-muted-foreground">
                  <tr>
                    ...
                    <th className="py-2 pr-3">Business Quality</th>
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.instrument.id} className="border-b align-top last:border-0">
                      ...
                      <td className="py-3 pr-3 font-medium">{score(scoreBusinessQuality(row.latestScore))}</td>
