import { buildEvaluation, scoreMacroFit, type RecommendationInput } from "./recommendationScoring";
...
export class BondEtfRecommendationService {
  ...
  evaluate(input: RecommendationInput) {
    ...
    const components: ScoreComponent[] = [
      ...
      { key: "yield_curve", label: "Yield Curve", score: scoreMacroFit(input.instrument, input.macroRegime, input.bondProfile), weight: 0.15, reason: "Yield-curve context supports the holding" },
      ...
      { key: "diversification", label: "Diversification", score: input.portfolioFit.score, weight: 0.1, reason: "Improves fixed-income diversification" }
    ];
