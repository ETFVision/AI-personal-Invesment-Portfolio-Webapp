...
The instrument detail Risk tab is a display-only surface over these stored fields. Its headline score and verdict display the characteristics-aligned risk component (`100 - risk_score`) with the same bands used by the Insights breakdown: below 45 = Elevated, below 70 = Moderate, and 70+ = Lower. This aligns the Risk tab and Insights tab display direction (`higher = lower risk`) without changing stored `risk_score`, `scoreRisk`, recommendation scoring, guardrails, confidence, or methodology anchors. Volatility-specific observations, volatility windows, drawdown windows, downside/tail diagnostics, metric freshness, and universe percentile context continue to read stored risk rows and price history.
