...
export class NewsSummaryEligibilityService {
  isEligible(item: ClassifiedNewsItem) {
    if (item.isDuplicate) return false;
    if (item.sourceProvider !== "gdelt") return true;

    const title = item.title.trim();
    if (title.length < 20) return false;
    const text = `${title} ${item.summary ?? ""} ${item.sourceName ?? ""}`.toLowerCase();
    ...
    const category = typeof item.providerMetadata.macroCategory === "string"
      ? item.providerMetadata.macroCategory
      : item.classification.affectedMacroCategories[0] ?? "";
  ...
  filter(items: ClassifiedNewsItem[]) {
