import { allocationPercent, finding, section, type PortfolioReviewInputContext } from "./portfolioReviewScoring";

export class AllocationReviewService {
  review({ dashboard }: PortfolioReviewInputContext) {
    const stocks = allocationPercent(dashboard.allocationByType, (label) => label.includes("stock"));
    const etfs = allocationPercent(dashboard.allocationByType, (label) => label.includes("etf"));
    const bonds = allocationPercent(dashboard.allocationByType, (label) => label.includes("bond"));
    const gold = allocationPercent(dashboard.allocationByType, (label) => label.includes("gold"));
    const crypto = allocationPercent(dashboard.allocationByType, (label) => label.includes("crypto"));
    const cash = dashboard.cashPercent;
    const findings = [
      stocks + etfs > 0.85 ? finding("watch", "Equity-heavy allocation", "Equities and ETFs dominate the portfolio, so market beta is likely the main driver.") : null,
    ...
    const score = 82
      - Math.max(0, stocks + etfs - 0.85) * 80
    ...
    return section(score, "Allocation is reviewed across cash, equities, fixed income, gold and crypto using current derived portfolio values.", findings, {
      stockAndEtfAllocation: stocks + etfs,
