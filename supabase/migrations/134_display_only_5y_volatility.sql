-- Add display-only 5Y volatility alongside existing long-horizon risk windows.
--
...
alter table instrument_risk_metrics add column if not exists volatility_5y numeric(28, 10);
...
begin
  ...
  return_stats as (
    select
      ...
      stddev_samp(daily_returns.daily_return) filter (where daily_returns.price_date >= latest_prices.latest_price_date - interval '5 years') * sqrt(252) as volatility_5y_raw,
  ...
  risk_returns as (
    select
      ...
      case when stats.history_start_date <= (latest_prices.latest_price_date - interval '5 years' + interval '30 days')::date then return_stats.volatility_5y_raw else null end as volatility_5y,
