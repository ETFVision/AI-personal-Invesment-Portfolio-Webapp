-- Precompute reusable daily/weekly return series for instrument risk metrics.
-- Raw instrument_prices remains the source of truth; this table removes repeated
-- lag/window work from every risk refresh.

create table if not exists instrument_daily_returns (
  instrument_id uuid not null references instruments(id) on delete cascade,
  price_date date not null,
  close_price numeric(28, 10) not null,
  previous_close_price numeric(28, 10),
  five_day_close_price numeric(28, 10),
...
create index if not exists idx_instrument_daily_returns_price_date
  on instrument_daily_returns (price_date desc);

create index if not exists idx_instrument_daily_returns_instrument_date_desc
  on instrument_daily_returns (instrument_id, price_date desc);

drop trigger if exists trg_instrument_daily_returns_updated_at on instrument_daily_returns;
create trigger trg_instrument_daily_returns_updated_at
  before update on instrument_daily_returns
  for each row execute function set_updated_at();

create or replace function refresh_instrument_daily_returns(target_instrument_ids uuid[] default null)
returns void
language plpgsql
as $$
begin
...
alter table instrument_daily_returns enable row level security;

drop policy if exists "users can read instrument daily returns" on instrument_daily_returns;
create policy "users can read instrument daily returns" on instrument_daily_returns
